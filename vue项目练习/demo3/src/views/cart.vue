            ...
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a href="javascript:;" @click="togCheckAll">
...
<script>
    ...
    //导入格式化数字的js文件  因为在current.js里 export function currency (..){..}  这里导入的时候就需要  import  {函数名}
    ...
    export default {
        data(){
          return {
              ...
              //控制模态框的显示和隐藏
            modalConfirm:false,
            delItem:'' //保存删除某个商品时的id
            //checkAllFlag:false   //默认没有全选购物车列表的商品  下面有个全局计算了，不需要再声明这个变量
          }
        },mounted: function () {
        this.init();
      }
      ,
      //进行全局计算，不用每次改变购物车列表的商品的时候，页面都要进行渲染  一旦有数据发生变化，就会实时的进行变化
      computed:{
          checkAllFlag(){
              return this.checkedCount == this.cartList.length;  //这里是使用checkedCount的属性，而不是方法
          },
        //下面这个函数计算：如果购物车商品的数量等于选中的数量，则判断是全部选中
          checkedCount(){
              var i = 0;
    ...
    // ,
      ...
      methods:{
        ...
        togCheckAll(){
            var flag = !this.checkAllFlag;  //用一个变量绑定来取反
          // 不能用 this.checkAllFlag  = !this.checkAllFlag  来取反 ，因为在compued全局计算那里会绑定checkAllFlag
          //当选中全选的话，全局计算那里checkAllFlag为true ,如果再次点击，上面全局计算判断
          //this.checkedCount == this.cartList.length,由于这个checkAllFlag的值又变为true
            this.cartList.forEach((item)=>{
