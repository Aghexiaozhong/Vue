        ...
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap">
              <span class="error error-show" v-show="errorTip">用户名过密码错误</span>
            </div>
            <ul>
              <li class="regi_form_input">
                <i class="icon IconPeople"></i>
                <input type="text" tabindex="1" name="loginname" v-model="userName" class="regi_form_input regi_login_input" placeholder="username">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i>
                <input type="password" tabindex="2" name="password" v-model="userPwd" class="regi_form_input regi_login_input" placeholder="password" @keyup.enter="login">
              </li>
            </ul>
          </div>
...
<script>
  //通过vuex的mapstore 里封装的各种函数来实现 compted的实时计算
      ...
      methods:{
        checkLogin(){
          ...
          login(){
            ...
            if(!this.userName || !this.userPwd){
              ...
              axios.post('/users/login',{
                ...
                if(res.status == '0'){
                  this.errorTip = false;
                  this.loginModalFlag = false;  //当登录成功的时候关闭登录弹框
          ...
          axios.get('/users/getCartCount').then(response=>{
              let res = response.data;
              if(res.status == '0'){
                //用vuex来同一管理购物车的商品总数量  这个得是初始化cartCount
